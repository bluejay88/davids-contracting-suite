...
const migrateState = (rawState) => {
  ...
  const adminPasswordSource = normalizeString(rawState?.appState?.settings?.adminPassword || rawState?.settings?.adminPassword)
    || starterAdminPassword;
  const staffPasswordSource = normalizeString(rawState?.appState?.settings?.staffPassword || rawState?.settings?.staffPassword)
    || starterStaffPassword;
...
const ensurePersistedState = async () => {
  await mkdir(dataDir, { recursive: true });

  if (!existsSync(stateFile)) {
    await writeFile(stateFile, JSON.stringify(createInitialPersistedState(), null, 2), "utf8");
  }
};
...
export const buildReminderFromRecord = (record) => ({
  id: `rem-${record.id}`,
