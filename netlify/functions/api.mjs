...
const sanitizePublicEstimateRequest = (record, quote) => {
  ...
  const breakdown = Array.isArray(quote?.breakdown) ? quote.breakdown.slice(0, 80).map((line) => ({
    taskId: cleanText(line?.taskId, 120), category: allowedCategories.has(line?.category) ? line.category : "handyman", taskName: cleanText(line?.taskName, 180), quantity: Math.min(100000, Math.max(0, Number(line?.quantity) || 0)), unitLabel: cleanText(line?.unitLabel, 80), scopeNote: cleanText(line?.scopeNote, 1000), lowLabor: safeMoney(line?.lowLabor), highLabor: safeMoney(line?.highLabor), lowMaterials: safeMoney(line?.lowMaterials), highMaterials: safeMoney(line?.highMaterials), lowTotal: safeMoney(line?.lowTotal), highTotal: safeMoney(line?.highTotal), laborHours: Math.min(100000, Math.max(0, Number(line?.laborHours) || 0)), unitPricing: { unitLabel: cleanText(line?.unitPricing?.unitLabel || line?.unitLabel, 80), lowLaborPerUnit: safeMoney(line?.unitPricing?.lowLaborPerUnit), highLaborPerUnit: safeMoney(line?.unitPricing?.highLaborPerUnit), lowMaterialsPerUnit: safeMoney(line?.unitPricing?.lowMaterialsPerUnit), highMaterialsPerUnit: safeMoney(line?.unitPricing?.highMaterialsPerUnit), lowInstalledPerUnit: safeMoney(line?.unitPricing?.lowInstalledPerUnit), highInstalledPerUnit: safeMoney(line?.unitPricing?.highInstalledPerUnit) }, roofingDetail: line?.roofingDetail && allowedCategories.has(line?.category) && line.category === "roofing" ? { coverageSqFt: safeMoney(line.roofingDetail.coverageSqFt), roofingSquares: safeMoney(line.roofingDetail.roofingSquares), laborPerSqFt: { low: safeMoney(line.roofingDetail.laborPerSqFt?.low), high: safeMoney(line.roofingDetail.laborPerSqFt?.high) }, materialsPerSqFt: { low: safeMoney(line.roofingDetail.materialsPerSqFt?.low), high: safeMoney(line.roofingDetail.materialsPerSqFt?.high) }, installedPerSqFt: { low: safeMoney(line.roofingDetail.installedPerSqFt?.low), high: safeMoney(line.roofingDetail.installedPerSqFt?.high) }, installedPerSquare: { low: safeMoney(line.roofingDetail.installedPerSquare?.low), high: safeMoney(line.roofingDetail.installedPerSquare?.high) } } : undefined, customaryIncludes: Array.isArray(line?.customaryIncludes) ? line.customaryIncludes.slice(0, 12).map((value) => cleanText(value, 300)) : [], customaryExcludes: Array.isArray(line?.customaryExcludes) ? line.customaryExcludes.slice(0, 12).map((value) => cleanText(value, 300)) : [], materials: [],
