...
import { submitFieldProjectNote, uploadFieldProjectMedia } from "../lib/api";

interface Props { appState: AppState; email: string; onOpenEstimate: () => void; onUpdateState: (state: AppState) => void; }

function ProjectCard({ project, onOpenEstimate, onSubmit }: { project: Project; onOpenEstimate: () => void; onSubmit: (projectId: string, note: string, attachments: unknown[]) => Promise<void> }) {
  const [note, setNote] = useState("");
  const [attachments, setAttachments] = useState<Array<{ id: string; name: string; mediaType: "image" | "video" | "document"; url: string; uploadedAt: string }>>([]);
  ...
  return <article className="field-workspace__project">
    ...
    <section className="field-workspace__activity"><strong>Recent activity</strong>{(project.activity || []).slice(0, 3).map((item) => <p key={item.id}><b>{item.authorName}</b> · {new Date(item.createdAt).toLocaleString()}<br />{item.body}</p>)}<textarea value={note} maxLength={4000} placeholder="Add a permanent field note for the Owner…" aria-label={`Field note for ${project.name}`} onChange={(event) => setNote(event.target.value)} /><label className="button button--ghost">Add project media<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,application/pdf" hidden onChange={(event) => { const file = event.currentTarget.files?.[0]; if (!file) return; setBusy(true); void uploadFieldProjectMedia(project.id, file).then((payload) => setAttachments((current) => [...current, payload.attachment])).finally(() => { setBusy(false); event.currentTarget.value = ""; }); }} /></label>{attachments.length ? <small>{attachments.length} attachment(s) staged for this note.</small> : null}<button className="button" disabled={busy || !note.trim()} onClick={() => void (async () => { setBusy(true); try { await onSubmit(project.id, note, attachments); setNote(""); setAttachments([]); } finally { setBusy(false); } })()}>{busy ? "Submitting…" : "Submit permanent note"}</button></section>
...
export function ContractorDashboard({ appState, email, onOpenEstimate, onUpdateState }: Props) {
  ...
  const submit = async (projectId: string, body: string, attachments: unknown[] = []) => { try { const payload = await submitFieldProjectNote(projectId, body, attachments); onUpdateState(payload.appState); setMessage("Field note submitted to the Owner."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to submit field note."); } };
